...
module.exports = {
        ...
        try {
            ...
            db.vendor.findOne({ where: {  email:  email} })
                ...
                .then(vendor => {
                    ...
                    return db.vendor_areas.bulkCreate(areaList)
                    }
                    return true
    ...
    async vendorAddProduct(req, res, next) {
        try {
            ...
            let id = productId
            db.vendor_products.findAll({ where: { supplierId: supplierId, productId: productId, unitSize: unitSize } })
                .then(data => {
                    if (!data.length>0) {
                        return db.vendor_products.create({supplierId: supplierId, productId: productId, unitSize: unitSize, price: buyerPrice})  
                    }else{
                        return db.vendor_products.update({ unitSize: unitSize ? unitSize: data.unitSize, price:buyerPrice ? buyerPrice: data.buyerPrice},{ where: { supplierId: supplierId, productId: productId}})  
                    }
                })
                .then(success => {
